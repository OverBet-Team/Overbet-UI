...
export interface LedgerSnapshot {
    ...
    isRunning: boolean;
    /**
     * Present when chip conservation is violated (sum of pnl ≠ 0).
...
export function computeLedgerSnapshot(
    ...
    const zeroSumError = sum !== 0 ? sum : undefined;
    if (zeroSumError !== undefined) {
        console.warn(`[computeLedgerSnapshot] Zero-sum violation: Σ PnL = ${sum} (must be exactly 0)`);
    }
