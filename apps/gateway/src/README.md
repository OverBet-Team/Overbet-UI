...
| `ledger.ts` | Ledger DB write module: `writeLedgerEntry`, `writeSessionEndEntries` (transactional), `loadLedgerEntries`, `loadConfirmations`, `loadDisputes`. Receives PrismaClient as parameter — never constructs its own. |
