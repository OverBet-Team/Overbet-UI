...
describe("toPlayerViewState", () => {
  it("includes cleanup phase and derives total/current pot amounts", () => {
    const players = [
    ...
    const gameState = {
      ...
      pot: 120,
      sidePots: [{ amount: 80 }, { amount: 40 }],
      dealerId: "u1",
      activePlayerId: "u2",
      players: [
        { id: "u1", stack: 1000, bet: 15 },
        { id: "u2", stack: 1000, bet: 35 },
      ],
    };
    ...
    expect(view!.totalPot).toBe(240);
    expect(view!.currentRoundAmount).toBe(50);
  });
  ...
  it("resets board to 5 slots with nulls for missing cards", () => {
    ...
    const gameState = { phase: "FLOP_BETTING", board: ["As", "Kh", "Qd"], pot: 100, players: [{ id: "u1", bet: 10 }] };
    const view = toPlayerViewState(players as any, gameState, "u1");
    expect(view).not.toBeNull();
    expect(view!.board).toHaveLength(5);
    expect(view!.board[0]).toBe("As");
    expect(view!.board[3]).toBeNull();
    expect(view!.totalPot).toBe(100);
    expect(view!.currentRoundAmount).toBe(10);
  });
});