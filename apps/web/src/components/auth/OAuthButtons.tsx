...
function GoogleIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
      ...
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 1.09 14.97 0 12 0 7.7 0 3.99 2.47 2.18 6.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
...
function Divider() {
  return (
    <div className="flex items-center gap-3 w-full my-1">
      ...
      <span className="text-sm text-white/40 select-none">or</span>
      <div className="flex-1 h-px bg-white/10" />
    </div>
  );
}

const PROVIDERS: { id: Provider; label: string; icon: React.ReactNode; style: React.CSSProperties; hoverBg: string }[] = [
  {
    id: "google",
...
export default function OAuthButtons({ redirectTo = "/" }: OAuthButtonsProps) {
  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      <Divider />
      {PROVIDERS.map(({ id, label, icon, style, hoverBg }) => {
        ...
        return (
          <button
            ...
            style={{
              ...style,
              ...(isHovered && !isDisabled ? { background: hoverBg } : {}),
              height: 44,
              borderRadius: 8,
