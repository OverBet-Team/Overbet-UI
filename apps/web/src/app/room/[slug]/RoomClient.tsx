...
import { InGameDesktopShell } from "@/components/poker/InGameDesktopShell";
import {
  toPlayerViewState,
  isBettingPhase,
  projectHeroTurnState,
} from "@/lib/hero-turn-projection";
...
function TurnTimerPill({ timer }: { timer: TurnTimer }) {
  ...
  const color =
    progress < 0.2 ? "#ef4444" : progress < 0.4 ? "#f97316" : "#22c55e";
  return (
    <div
...
export default function RoomClient({ slug, initialRoom }: RoomProps) {
  ...
  const [approvedSeatOverride, setApprovedSeatOverride] =
  ...
  const [nowMs, setNowMs] = useState(() => Date.now()),
    shouldTickNowForHeroTimer =
      !!userId &&
      isBettingPhase(gameState?.phase) &&
      gameState?.activePlayerId === userId;
  ...
  useEffect(() => {
    if (!shouldTickNowForHeroTimer) return;
    setNowMs(Date.now());
    const interval = setInterval(() => setNowMs(Date.now()), 100);
    return () => clearInterval(interval);
  }, [shouldTickNowForHeroTimer, turnTimer?.expiresAt]);
  ...
  useEffect(() => {
    ...
    socketInstance.on(
      ...
      (snapshot: {
        ...
        setPlayers((prev) => {
          ...
          const me = prev.find(
            (p) => p.id === userId && p.seatIndex !== undefined,
          );
    ...
    socketInstance.on("EVENT_SEAT_APPROVED", (data: any) => {
      ...
      setPendingRequests((prev) =>
        prev.filter((r) => r.playerId !== data.playerId),
      );
      ...
      if (data.playerId === userId) {
        setApprovedSeatOverride(
          shouldApplyApprovedSeatOverride ? nextApprovedSeatOverride : null,
        );
      }
    });
    ...
    socketInstance.on("EVENT_STATE_UPDATE", (snapshot: { state: any }) => {
      ...
      if (state.phase !== "LOBBY") {
        setRoom((prev) =>
          prev.status !== "INGAME" ? { ...prev, status: "INGAME" } : prev,
        );
      }
      setTurnTimer((prev) => {
      ...
      if (state.phase === "CLEANUP") {
        ...
        if (
          myEnginePlayer &&
          (myEnginePlayer.stack === 0 || myEnginePlayer.chips === 0)
        ) {
          setIsBusted(true);
        }
      ...
      if (state.phase === "CLEANUP" || state.phase === "SHOWDOWN") {
        ...
        if (winnerPlayer) {
          winnerInfo = {
            ...
            winnerCards: Array.isArray(winnerPlayer.holeCards)
              ? winnerPlayer.holeCards
              : winnerPlayer.cards,
          };
        } else if (latestWinningEvent?.winnerId) {
          const wp = state.players?.find(
            (x: any) => x.id === latestWinningEvent?.winnerId,
          );
          const displayName =
            wp?.displayName ||
            wp?.username ||
            wp?.id ||
            latestWinningEvent.winnerId;
          winnerInfo = {
            name: displayName,
    ...
    socketInstance.on("EVENT_STATE_SNAPSHOT", (snapshot: { state: any }) => {
      ...
      setTurnTimer((prev) => {
        ...
        return prev && snapshot.state.activePlayerId === prev.playerId
          ? prev
          : null;
      });
    });
  ...
  const handleRebuy = (amount: number, _displayName: string) => {
    ...
    socket?.emit("INTENT_SEAT_REQUEST", {
      ...
      stack: amount,
      displayName:
        myDisplayName || myPlayer?.username || `Player_${userId.slice(0, 4)}`,
    });
  ...
  const mappedPlayers: PlayerData[] = [
    ...
    !players.some((p) => p.id === approvedSeatOverride.id)
      ? [approvedSeatOverride]
      : []),
  ];
    ...
    >
      <div
        style={{
          ...
          borderRadius: isPortraitMobile ? "18px 18px 0 0" : 24,
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          >
            ...
            >
              SHA-256 hash generated before cards were dealt — proves the deck
              order is fixed.
            </p>
          </div>
          ...
          >
            Even the server host cannot see your cards until they are revealed
            at showdown.
          </p>
        </div>
      </div>
  ...
  const HostControlPanel = ({ floating = false }: { floating?: boolean }) => {
    ...
    const canStart =
      players.length >= 2 && (!gameState || gameState.phase === "LOBBY");
    const canPause =
      !!gameState &&
      gameState.phase !== "LOBBY" &&
      gameState.phase !== "CLEANUP" &&
      !isPaused;
    const showStartResume = isPaused || canStart;

    return (
      <div
        style={{
          ...(floating
            ? { position: "absolute", top: 8, left: 8, zIndex: 24 }
            : {}),
                ...
                {pendingRequests.map((req) => (
                  ...
                  >
                    <div>
                      ...
                      >
                        {req.displayName ||
                          `Player_${req.playerId.slice(0, 4)}`}
                      </div>
                      <div
                        style={{
                          color: "rgba(255,255,255,0.5)",
                      ...
                      >
                        <span>
                          {req.requestType === "REBUY" ? "Re-buy" : "Seat"}{" "}
                          {req.seatIndex + 1} ·
                        </span>
                        <ChipAmount
                          amount={req.stack}
  ...
  const mySeat =
    ...
    (approvedSeatOverride && approvedSeatOverride.id === userId
      ? approvedSeatOverride
      : undefined);
  if (!mySeat) {
    return (
                ...
                {[
                  ...
                  },
                  {
                    ...
                    value: (
                      ...
                      >
                        <ChipAmount
                          amount={room.settings?.smallBlind || 10}
                          iconSize={11}
                          amountStyle={{
                            color: "inherit",
                            fontSize: 12,
                        ...
                        <span>/</span>
                        <ChipAmount
                          amount={room.settings?.bigBlind || 20}
                          iconSize={11}
                          amountStyle={{
                            color: "inherit",
                            fontSize: 12,
                  ...
                  },
                  {
                    ...
                    value: (
                      ...
                      >{`${room.settings?.turnTimeout || 30}s`}</span>
                    ),
                  },
                  {
                    label: "Time Bank",
                    value: (
                      ...
                      >{`${room.settings?.timeBank || 30}s`}</span>
                    ),
                  },
                  {
                    label: "Auto-Start",
                    value: (
                      ...
                      >{`${room.settings?.autoStartDelay || 5}s`}</span>
                    ),
                  },
                ].map(({ label, value }) => (
                  <div
  ...
  const myStack =
    myPlayerInfo?.stack ?? myPlayerInfo?.chips ?? myLobbyPlayer?.chips ?? null;
  const heroTurnProjection = projectHeroTurnState({
    userId,
    heroStatus: myPlayerInfo?.status,
    phase: gameState?.phase,
    activePlayerId: gameState?.activePlayerId,
    turnTimer,
    nowMs,
  });
  const projectedGameStateForView =
    gameState &&
    gameState.activePlayerId !== heroTurnProjection.projectedViewActivePlayerId
      ? {
          ...gameState,
          activePlayerId: heroTurnProjection.projectedViewActivePlayerId,
        }
      : gameState;
  const isActivePlayer = heroTurnProjection.heroCanAct;
  ...
  const formattedPhaseLabel =
    gameState?.phase?.replaceAll("_", " ") || "Waiting";
  const heroName =
    myDisplayName.trim() ||
    myPlayerInfo?.username?.trim() ||
    myLobbyPlayer?.username?.trim() ||
    "";
  const heroInitials = (() => {
    const parts: string[] = heroName.split(/\s+/).filter(Boolean).slice(0, 2);
    if (parts.length > 0) {
      return parts.map((part) => part[0]?.toUpperCase() ?? "").join("") || "OB";
    }
    return heroName.charAt(0).toUpperCase() || "OB";
  })();
  const tableContent = (
    <div
      style={{
        ...
        flexDirection: "column",
        position: "relative",
        paddingTop: isPortraitMobile ? 36 : 0,
      }}
    >
      <div
      ...
      {(() => {
        const viewState = toPlayerViewState(
          mappedPlayers,
          projectedGameStateForView,
          userId,
        );
        if (viewState) {
          const isCleanup = gameState?.phase === "CLEANUP";
          const board5 = Array.from(
            { length: 5 },
            (_, i) => (gameState?.board ?? [])[i] ?? null,
          );
          const hasUnrevealedCards = board5.some((c: string | null) => !c);
          const showShowAllButton =
            isCleanup && !cleanupShowAllRevealed && hasUnrevealedCards;
          return (
            <div
              style={{
                flex: 1,
            ...
            >
              ...
              {showShowAllButton && (
                <button
                  ...
                  style={{
                    position: isPortraitMobile ? "fixed" : "absolute",
                    left: 20,
                    bottom: isPortraitMobile
      ...
      {isPortraitMobile && (
        <div
          style={{
            ...
            padding: "6px 10px",
            background:
              "linear-gradient(180deg, rgba(11,9,20,0.88) 0%, rgba(11,9,20,0.2) 100%)",
      ...
      {!gameState && (
        <div
          style={{
            position: "absolute",
            bottom: isPortraitMobile ? undefined : 36,
  ...
  return (
    ...
    >
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {heroTurnProjection.showHeroTimer && turnTimer && (
            ...
            <>
              ...
              <div
                ...
                style={{
                  ...
                  justifyContent: "center",
                  background:
                    "linear-gradient(160deg, #4f34b3 0%, #241a46 100%)",
                  border: "1px solid rgba(255,255,255,0.08)",
                  color: "#fff",
                  fontSize: 13,
                  fontWeight: 800,
        ...
        >
          <div
            style={{
              width: "100%",
              maxWidth: isPortraitMobile ? "100%" : 420,
              maxHeight: isPortraitMobile ? "78dvh" : "70vh",
              overflow: "hidden",
              background: "rgba(16,13,28,0.98)",
              borderRadius: isPortraitMobile ? "18px 18px 0 0" : 20,
              border: "1px solid rgba(255,255,255,0.1)",
              boxShadow: "0 24px 80px rgba(0,0,0,0.6)",
            }}
      ...
      {showHelpOverlay && (
        ...
        >
          <div
            style={{
              width: "100%",
              maxWidth: isPortraitMobile ? "100%" : 360,
              padding: isPortraitMobile ? "18px 16px 24px" : 24,
              background: "rgba(16,13,28,0.98)",
              borderRadius: isPortraitMobile ? "18px 18px 0 0" : 20,
              border: "1px solid rgba(255,255,255,0.1)",
              boxShadow: "0 24px 80px rgba(0,0,0,0.6)",
          ...
          >
            ...
            >
              <div>
                {isPortraitMobile ? (
                  "Use the bottom action tray for Fold, Check/Call, Raise, and All-In."
                ) : (
                  <>
                    <kbd
                      style={{
              ...
              <div>
                {isPortraitMobile ? (
                  "Tap Log to inspect hand history while playing."
                ) : (
                  <>
                    <kbd
                      style={{
      ...
      <BuyInModal
        ...
        isGuest={true}
        initialDisplayName={
          myDisplayName || players.find((p) => p.id === userId)?.username || ""
        }
        mode="rebuy"
      />
    </div>
